 ...
 * shielded memory must cause a ArenaAccess.
...
static void protLower(Arena arena, Seg seg, AccessSet mode)
{
  ...
  if(seg->pm & mode) {
    seg->pm &= ~mode;
    ProtSet(ArenaSegBase(arena, seg), ArenaSegLimit(arena, seg), seg->pm);
  }
}
...
static void sync(Arena arena, Seg seg)
{
  ...
  if(seg->pm != seg->sm) {
    ProtSet(ArenaSegBase(arena, seg), ArenaSegLimit(arena, seg), seg->sm);
    seg->pm = seg->sm;
    /* inv.prot.shield */
  }
}
