/* impl.c.vmsu: VIRTUAL MEMORY MAPPING FOR SUNOS 4
 *
 * $HopeName: MMsrc!vmsu.c(MMdevel_assertid.2) $
 * Copyright (C) 1995 Harlequin Group, all rights reserved
 *
 * Design: design.mps.vm
 *
 * This is the implementation of the virtual memory mapping interface
 * (vm.h) for SunOS 4.
 *
 * mmap(2) is used to reserve address space by creating a mapping to
 * /etc/passwd with page access none.  mmap(2) is used to map pages
 * onto store by creating a copy-on-write mapping to /dev/zero.
 *
 * Experiments have shown that attempting to reserve address space
 * by mapping /dev/zero results in swap being reserved.  This
 * appears to be a bug, so we work round it by using /etc/passwd,
 * the only file we can think of which is pretty much guaranteed
 * to be around.
 *
 * .assume.not-last: The implementation of VMCreate assumes that
 *   mmap() will not choose a region which contains the last page
 *   in the address space, so that the limit of the mapped area
 *   is representable.
 *
 * .assume.size: The maximum size of the reserved address space
 *   is limited by the range of "int".  This will probably be half
 *   of the address space.
 *
 * .assume.mmap.err: ENOMEM is the only error we really expect to
 *   get from mmap.  The others are either caused by invalid params
 *   or features we don't use.  See mmap(2) for details.
 */

#include "mpm.h"

#ifndef MPS_OS_SU
#error "vmsu.c is SunOS 4 specific, but MPS_OS_SU is not set"
#endif
#ifdef VM_RM
#error "vmsu.c compiled with VM_RM set"
#endif /* VM_RM */

#include <sys/types.h>
#include <sys/mman.h>
#include <fcntl.h>
#include <errno.h>
#include <sys/errno.h>

SRCID(vmsu, "$HopeName: MMsrc!vmsu.c(MMdevel_assertid.2) $");


/* Fix up unprototyped system calls.  */
/* Note that these are not fixed up by std.h because that only fixes */
/* up discrepancies with ANSI. */

extern int close(int fd);
extern int munmap(caddr_t addr, int len);
extern int getpagesize(void);

#define SpaceVM(space)  (&(space)->arenaStruct.vmStruct)

Align VMAlign(void)
{
  Align align;

  align = (Align)getpagesize();
  AVER(0xF3540000, SizeIsP2(align));

  return align;
}


Bool VMCheck(VM vm)
{
  CHECKS(0xF3540001, VM, vm);
  CHECKL(0xF3540002, vm->zero_fd >= 0);
  CHECKL(0xF3540003, vm->none_fd >= 0);
  CHECKL(0xF3540004, vm->zero_fd != vm->none_fd);
  CHECKL(0xF3540005, vm->base != 0);
  CHECKL(0xF3540006, vm->limit != 0);
  CHECKL(0xF3540007, vm->base < vm->limit);
  CHECKL(0xF3540008, vm->mapped <= vm->reserved);
  CHECKL(0xF3540009, SizeIsP2(vm->align));
  CHECKL(0xF354000A, AddrIsAligned(vm->base, vm->align));
  CHECKL(0xF354000B, AddrIsAligned(vm->limit, vm->align));
  return TRUE;
}


Res VMCreate(Space *spaceReturn, Size size, Addr base)
{
  caddr_t addr;
  Align align;
  int zero_fd;
  int none_fd;
  Space space;
  VM vm;

  align = VMAlign();

  AVER(0xF354000C, spaceReturn != NULL);
  AVER(0xF354000D, SizeIsAligned(size, align));
  AVER(0xF354000E, size != 0);
  AVER(0xF354000F, size <= INT_MAX); /* see .assume.size */
  AVER(0xF3540010, base == NULL);

  zero_fd = open("/dev/zero", O_RDONLY);
  if(zero_fd == -1)
    return ResFAIL;
  none_fd = open("/etc/passwd", O_RDONLY);
  if(none_fd == -1) {
    close(zero_fd);
    return ResFAIL;
  }

  /* Map in a page to store the descriptor on. */
  addr = mmap((caddr_t)0, SizeAlignUp(sizeof(SpaceStruct), align),
              PROT_READ | PROT_WRITE, MAP_PRIVATE,
              zero_fd, (off_t)0);
  if((int)addr == -1) {
    int e = errno;
    AVER(0xF3540011, e == ENOMEM); /* .assume.mmap.err */
    close(none_fd);
    close(zero_fd);
    if(e == ENOMEM)
      return ResMEMORY;
    else
      return ResFAIL;
  }
  space = (Space)addr;
  vm = SpaceVM(space);

  vm->zero_fd = zero_fd;
  vm->none_fd = none_fd;
  vm->align = align;

  /* See .assume.not-last. */
  addr = mmap((caddr_t)0, size, PROT_NONE, MAP_SHARED, none_fd, (off_t)0);
  if((int)addr == -1) {
    int e = errno;
    AVER(0xF3540012, e == ENOMEM); /* .assume.mmap.err */
    close(none_fd);
    close(zero_fd);
    if(e == ENOMEM)
      return ResRESOURCE;
    else
      return ResFAIL;
  }

  vm->base = (Addr)addr;
  vm->limit = AddrAdd(vm->base, size);
  vm->reserved = size;
  vm->mapped = (Size)0;

  vm->sig = VMSig;

  AVERT(0xF3540013, VM, vm);

  *spaceReturn = space;
  return ResOK;
}


void VMDestroy(Space space)
{
  int r;
  VM vm = SpaceVM(space);

  AVERT(0xF3540014, VM, vm);
  AVER(0xF3540015, vm->mapped == (Size)0);

  /* This appears to be pretty pointless, since the space descriptor */
  /* page is  about to vanish completely.  However, munmap might fail */
  /* for some reason, and this would ensure that it was still */
  /* discovered if sigs were being checked. */
  vm->sig = SigInvalid;

  close(vm->zero_fd);
  close(vm->none_fd);
  r = munmap((caddr_t)vm->base, (int)AddrOffset(vm->base, vm->limit));
  AVER(0xF3540016, r == 0);
  r = munmap((caddr_t)space,
             (int)SizeAlignUp(sizeof(SpaceStruct), vm->align));
  AVER(0xF3540017, r == 0);
}


Addr VMBase(Space space)
{
  VM vm = SpaceVM(space);
  AVERT(0xF3540018, VM, vm);
  return vm->base;
}

Addr VMLimit(Space space)
{
  VM vm = SpaceVM(space);
  AVERT(0xF3540019, VM, vm);
  return vm->limit;
}


Size VMReserved(Space space)
{
  VM vm = SpaceVM(space);
  AVERT(0xF354001A, VM, vm);
  return vm->reserved;
}

Size VMMapped(Space space)
{
  VM vm = SpaceVM(space);
  AVERT(0xF354001B, VM, vm);
  return vm->mapped;
}


Res VMMap(Space space, Addr base, Addr limit)
{
  VM vm = SpaceVM(space);
  Size size;

  AVERT(0xF354001C, VM, vm);
  AVER(0xF354001D, sizeof(int) == sizeof(Addr));
  AVER(0xF354001E, base < limit);
  AVER(0xF354001F, base >= vm->base);
  AVER(0xF3540020, limit <= vm->limit);
  AVER(0xF3540021, AddrOffset(base, limit) <= INT_MAX);
  AVER(0xF3540022, AddrIsAligned(base, vm->align));
  AVER(0xF3540023, AddrIsAligned(limit, vm->align));

  /* Map /dev/zero onto the area with a copy-on-write policy.  This */
  /* effectively populates the area with zeroed memory. */

  size = AddrOffset(base, limit);

  if((int)mmap((caddr_t)base, (int)size,
               PROT_READ | PROT_WRITE | PROT_EXEC,
               MAP_PRIVATE | MAP_FIXED,
               vm->zero_fd, (off_t)0) == -1) {
    AVER(0xF3540024, errno == ENOMEM); /* .assume.mmap.err */
    return ResMEMORY;
  }

  vm->mapped += size;

  return ResOK;
}


void VMUnmap(Space space, Addr base, Addr limit)
{
  VM vm = SpaceVM(space);
  Size size;
  caddr_t addr;

  AVERT(0xF3540025, VM, vm);
  AVER(0xF3540026, sizeof(int) == sizeof(Addr));
  AVER(0xF3540027, base < limit);
  AVER(0xF3540028, base >= vm->base);
  AVER(0xF3540029, limit <= vm->limit);
  AVER(0xF354002A, AddrIsAligned(base, vm->align));
  AVER(0xF354002B, AddrIsAligned(limit, vm->align));

  /* Map /etc/passwd onto the area, allowing no access.  This */
  /* effectively depopulates the area from memory, but keeps */
  /* it "busy" as far as the OS is concerned, so that it will not */
  /* be re-used by other calls to mmap which do not specify MAP_FIXED. */

  size = AddrOffset(base, limit);

  addr = mmap((caddr_t)base, (int)size,
              PROT_NONE, MAP_SHARED, vm->none_fd, (off_t)0);
  AVER(0xF354002C, (int)addr != -1);

  vm->mapped -= size;
}

