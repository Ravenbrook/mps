 ...
 *
 *  $HopeName: MMsrc!vmnt.c(MMdevel_remem.1) $
 *
...
SRCID(vmnt, "$HopeName: MMsrc!vmnt.c(MMdevel_remem.1) $");
...
Res VMCreate(Space *spaceReturn, size_t size)
{
  ...
  vm->align = align;
  vm->base = (Addr)base;
  vm->limit = AddrAdd((Addr)base, size);
  vm->reserved = size;
  vm->mapped = 0;
...
void VMUnmap(Space space, Addr base, Addr limit)
{
  ...
  b = VirtualFree((LPVOID)base,
      (DWORD)AddrOffset(base, limit), MEM_DECOMMIT);
