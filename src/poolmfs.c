 ...
 *
 * $HopeName: MMsrc!poolmfs.c(trunk.17) $
...
SRCID(poolmfs, "$HopeName: MMsrc!poolmfs.c(trunk.17) $");
...
static Res MFSInit(Pool pool, va_list arg)
{
  ...
  extendBy = SizeAlignUp(extendBy, ArenaAlignment(arena));
...
#define SUB(b, s, i)    ((Header)AddrAdd(b, (s)*(i)))
    ...
    for(i=0; i<unitsPerSeg; ++i)
    {
      ...
      AVER(AddrAdd((Addr)header, unitSize) <= ArenaSegLimit(arena, seg));
      header->next = next;
      next = header;
    }
...
Bool MFSCheck(MFS mfs)
{
  ...
  CHECKL(SizeIsAligned(mfs->extendBy, ArenaAlignment(arena)));
  ...
  if(mfs->freeList != NULL) {
    ...
    /* Can't do this check because MFSCheck is called from ArenaCheck. */
