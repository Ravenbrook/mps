...
# $HopeName: MMsrc!eventgen.pl(MMdevel_gavinm_160033.1) $
#
...
$HopeName = '$HopeName: MMsrc!eventgen.pl(MMdevel_gavinm_160033.1) $';
$HopeName =~ s/\$//;
...
print H "#define EVENT_0(type) \\
  BEGIN \\
    if((EVENT_ALL || Event ## type ## Always) && \\
       BS_IS_MEMBER(EventKindControl, ((Index)Event ## type ## Kind))) { \\
...
foreach $format (sort(keys(%Formats))) {
  ...
  print H "  BEGIN \\\n";
  print H "    if((EVENT_ALL || Event ## type ## Always) && \\\n";
  print H "       BS_IS_MEMBER(EventKindControl, \\\n";
  print H "                    ((Index)Event ## type ## Kind))) { \\\n";
