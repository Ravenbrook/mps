 ...
 *
 *  $HopeName: MMsrc!proto1.c(MMdevel_assertid.2) $
...
SRCID(proto1, "$HopeName: MMsrc!proto1.c(MMdevel_assertid.2) $");
...
static void sigHandle(int sig, siginfo_t *info, void *context)
{
  ...
  AVER(0xB6010000, sig == SIGSEGV);
  AVER(0xB6010001, info != NULL);
...
void ProtSet(Addr base, Addr limit, AccessSet mode)
{
  ...
  AVER(0xB6010007, sizeof(size_t) == sizeof(Addr));
  AVER(0xB6010008, base < limit);
  AVER(0xB6010009, base != 0);
  AVER(0xB601000A, AddrOffset(base, limit) <= INT_MAX);     /* should be redundant */
...
void ProtTramp(void **resultReturn, void *(*f)(void *, size_t),
               void *p, size_t s)
{
  AVER(0xB601000D, f != NULL);
  AVER(0xB601000E, resultReturn != NULL);
