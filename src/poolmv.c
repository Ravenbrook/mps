 ...
 *
 * $HopeName: MMsrc!poolmv.c(trunk.19) $
...
static Bool MVSpanCheck(MVSpan span)
{
  ...
  CHECKL(span->base.base == ArenaSegBase(PoolArena(MVPool(span->mv)), span->seg));
  CHECKL(span->limit.limit == ArenaSegLimit(PoolArena(MVPool(span->mv)), span->seg));
...
static Res MVAlloc(Addr *pReturn, Pool pool, Size size)
{
  ...
  RingInit(&span->spans);
  span->base.base = span->base.limit = ArenaSegBase(arena, span->seg);
  span->limit.base = span->limit.limit = ArenaSegLimit(arena, span->seg);
...
static void MVFree(Pool pool, Addr old, Size size)
{
  ...
  /* onto the span. */
  b = ArenaSegOfAddr(&seg, PoolArena(pool), old);
  AVER(b);
  span = (MVSpan)seg->p;
  AVERT(MVSpan, span);
