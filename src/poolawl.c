 ...
 *
 * $HopeName: !poolawl.c(MM_dylan_sunflower.13) $
...
SRCID(poolawl, "$HopeName: !poolawl.c(MM_dylan_sunflower.13) $");
...
notFinished:
  ...
  if(SegWhite(seg) != RefSetEMPTY) {
    ...
    ss->fixed = RefSetUnion(ss->fixed,
                            RefSetInter(SegSummary(seg), ss->white));
  }
