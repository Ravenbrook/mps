;;; compiled-test.sc -- tests from R5RS
;;;
;;; This file contains test code derived directly from R5RS.  It ensures that all the
;;; functions correctly evaluate the examples in R5RS.
;;;
;;; DOCUMENT HISTORY
;;;
;;; 2004-07-27  RB  Added document history and Perforce Id keyword.
;;;
;;; 2004-08-05  RB  Branched to test compiled code, rather than calling "eval"
;;; which only tests the interpreter.
;;;
;;; $Id$
...
(check (call-with-current-continuation (lambda (exit) (for-each (lambda (x) (if (negative? x) (exit x))) '(54 0 37 -3 245 19)) #t)) '-3)
...
(check `#(10 5 ,(abs -4) ,@(map abs '(16 -9)) 8) '#(10 5 4 16 9 8))
...
(check (let loop ((numbers '(3 -2 1 6 -5)) (nonneg '()) (neg '())) (cond ((null? numbers) (list nonneg neg)) ((>= (car numbers) 0) (loop (cdr numbers) (cons (car numbers) nonneg) neg)) ((< (car numbers) 0) (loop (cdr numbers) nonneg (cons (car numbers) neg))))) '((6 1 3) (-5 -2)))



(display "No defects found.")
(newline)
